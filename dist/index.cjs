...
ordered_binary = __toESM(ordered_binary, 1);
...
async function withBackupDirLock(backupDir, fn) {
	const token = tryFileLock((0, node_path.join)(backupDir, ".backup.lock"));
...
async function pruneOrphanedTransactionLogs(backupDir) {
	const logsRoot = (0, node_path.join)(backupDir, TRANSACTION_LOGS_DIRNAME);
	let ids;
	try {
		ids = await (0, node_fs_promises.readdir)(logsRoot);
	} catch (err) {
	...
	await Promise.all(ids.filter((id) => !surviving.has(id)).map((id) => (0, node_fs_promises.rm)((0, node_path.join)(logsRoot, id), {
		recursive: true,
		force: true
	})));
}
...
async function restoreTransactionLogs(backupDir, dbDir, backupId) {
	...
	const logsSrc = (0, node_path.join)(logsRoot, String(id));
	if (!await exists(logsSrc)) return;
	const logsDest = (0, node_path.join)(dbDir, TRANSACTION_LOGS_DIRNAME);
	await (0, node_fs_promises.rm)(logsDest, {
		recursive: true,
		force: true
	});
	await (0, node_fs_promises.cp)(logsSrc, logsDest, {
		recursive: true,
...
const backups = {
	...
	*/
	async restore(backupDir, dbDir, options) {
		if ((0, node_path.resolve)(backupDir) === (0, node_path.resolve)(dbDir)) throw new Error("Backup directory and database directory must be different");
		const walDir = options?.walDir ?? dbDir;
		await (0, node_fs_promises.mkdir)(dbDir, { recursive: true });
		if (walDir !== dbDir) await (0, node_fs_promises.mkdir)(walDir, { recursive: true });
	...
	*/
	async delete(backupDir, backupId) {
		return withBackupDirLock(backupDir, async () => {
			...
			try {
				await (0, node_fs_promises.rm)((0, node_path.join)(backupDir, TRANSACTION_LOGS_DIRNAME, String(backupId)), {
					recursive: true,
					force: true
				});
			} catch {}
...
var Store = class {
	...
	*/
	...
	getRange(context, options) {
		...
		return new _harperfast_extended_iterable.ExtendedIterable(new DBIterator(new NativeIterator(context, flags, startKeyEnd, endKeyStart, endKeyEnd, advancedOptions), this, includeValues, options?.limit));
	}
